    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center">
      <div className="w-[640px] rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {emailsToInvite.map(participant => {
            return (
              <div key={participant.email} className="py-1.5 px-2.5 rounded-md bg-zinc-800 flex items-center gap-2">
                <span className="text-zinc-300">{participant.email}</span>
                <button type="button">
                  <X onClick={() => removeEmailFromInvites(participant.email)} className="size-4 text-zinc-400" />
                </button>
              </div>
              )
            }
